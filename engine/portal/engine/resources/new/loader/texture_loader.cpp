...
TextureLoader::TextureLoader(ResourceRegistry& registry, renderer::vulkan::VulkanContext& context): ResourceLoader(registry), context(context)
{
...
Resource* TextureLoader::load(const SourceMetadata& meta, const ResourceSource& source)
{
    ...
    auto* texture = new renderer::vulkan::VulkanTexture(meta.resource_id);
    texture->initialize(spec, {image_data, size}, context);
...
void TextureLoader::create_standalone_texture(const StringId& id, std::span<uint32_t> data, vk::Extent3D extent)
{
    ...
    // TODO: should we use new?
    auto* texture = new renderer::vulkan::VulkanTexture(id);
    texture->initialize(spec, {data.data(), data.size() * sizeof(uint32_t)}, context);
    texture->set_state(ResourceState::Loaded);

    registry.resources[id.id] = texture;
}
