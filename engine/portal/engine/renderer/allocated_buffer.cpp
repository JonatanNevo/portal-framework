...
{
AllocatedBuffer AllocatedBuffer::create_staging_buffer(vk::raii::Device& device, const vk::DeviceSize size, const void* data)
{
...
AllocatedBuffer BufferBuilder::build(vk::raii::Device& device) const
{
    return {device, *this};
}

std::shared_ptr<AllocatedBuffer> BufferBuilder::build_shared(vk::raii::Device& device) const
{
...
uint64_t AllocatedBuffer::get_device_address() const
{
    ...
    return get_device().getBufferAddress(info);
}
...
AllocatedBuffer::AllocatedBuffer(vk::raii::Device& device, const BufferBuilder& builder): Allocated(
