...
static const ImGuiWindowFlags NOTIFY_DEFAULT_TOAST_FLAGS = ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoNav
    | ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoFocusOnAppearing;

#define NOTIFY_NULL_OR_EMPTY(str)        (!str || !strlen(str))
#define NOTIFY_FORMAT(fn, format, ...)    if (format) {va_list args; va_start(args, format); fn(format, args __VA_OPT__(,) __VA_ARGS__); va_end(args);}
...
public:
     ...
     * 
     * @param new_type The type of the toast notification.
     */
    inline void setType(const ImGuiToastType& new_type)
    {
        IM_ASSERT(new_type < ImGuiToastType::COUNT);
        this->type = new_type;
    };
     ...
     * 
     * @param new_flags ImGui window flags to set.
    */
    inline void setWindowFlags(const ImGuiWindowFlags& new_flags)
    {
     ...
     * 
     * @param newOnButtonPress std::fuction or lambda expression, which contains the code for execution.
    */
    inline void setOnButtonPress(const std::function<void()>& newOnButtonPress)
    {
        this->onButtonPress = newOnButtonPress;
    }
...
inline void RenderNotifications()
{
    ...
    for (size_t i = 0; i < notifications.size(); ++i)
    {
        ...
        SetNextWindowPos(
            ImVec2(monitor.WorkPos.x + monitor.WorkSize.x - NOTIFY_PADDING_X, monitor.WorkPos.y + monitor.WorkSize.y - NOTIFY_PADDING_Y - height),
            ImGuiCond_Always,
            ImVec2(1.0f, 1.0f)
        );
#else
        ...
        SetNextWindowPos(
            ImVec2(mainWindowPos.x + mainWindowSize.x - NOTIFY_PADDING_X, mainWindowPos.y + mainWindowSize.y - NOTIFY_PADDING_Y - height),
            ImGuiCond_Always,
            ImVec2(1.0f, 1.0f)
        );
#endif
