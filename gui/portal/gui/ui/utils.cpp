//
// Created by Jonatan Nevo on 04/02/2025.
//
...
namespace portal::ui
{
void shift_cursor_x(const float distance) { ImGui::SetCursorPosX(ImGui::GetCursorPosX() + distance); }

void shit_cursor_y(const float distance) { ImGui::SetCursorPosY(ImGui::GetCursorPosY() + distance); }
...
ImRect get_item_rect() { return {ImGui::GetItemRectMin(), ImGui::GetItemRectMax()}; }

ImRect rect_expand(const ImRect& rect, const float x, const float y)
{
    ImRect result = rect;
    result.Min.x -= x;
    result.Min.y -= y;
...
ImRect rect_offset(const ImRect& rect, const ImVec2 xy) { return rect_offset(rect, xy.x, xy.y); }
...
void draw_button_image(
    ...
    const ImU32 tint_normal,
    const ImU32 tint_hovered,
    const ImU32 tint_pressed,
    ...
    const ImVec2 rect_max)
{
    ...
    if (ImGui::IsItemActive())
        drawList->AddImage(
            reinterpret_cast<intptr_t>(image_pressed->get_descriptor_set()), rect_min, rect_max, ImVec2(0, 0), ImVec2(1, 1), tint_pressed);
    else if (ImGui::IsItemHovered())
        drawList->AddImage(
            reinterpret_cast<intptr_t>(image_hovered->get_descriptor_set()), rect_min, rect_max, ImVec2(0, 0), ImVec2(1, 1), tint_hovered);
    else
        drawList->AddImage(
            reinterpret_cast<intptr_t>(image_normal->get_descriptor_set()), rect_min, rect_max, ImVec2(0, 0), ImVec2(1, 1), tint_normal);
}
...
{
    draw_button_image(image_normal, image_hovered, image_pressed, tint_normal, tint_hovered, tint_pressed, rectangle.Min, rectangle.Max);
}
...
void draw_button_image(
    ...
    const ImVec2 rect_max)
{
    draw_button_image(image, image, image, tint_normal, tint_hovered, tint_pressed, rect_min, rect_max);
}

void draw_button_image(
    const std::shared_ptr<portal::Image>& image, const ImU32 tint_normal, const ImU32 tint_hovered, const ImU32 tint_pressed, const ImRect rectangle)
{
    draw_button_image(image, image, image, tint_normal, tint_hovered, tint_pressed, rectangle.Min, rectangle.Max);
}
...
{
    draw_button_image(
        image_normal, image_hovered, image_pressed, tint_normal, tint_hovered, tint_pressed, ImGui::GetItemRectMin(), ImGui::GetItemRectMax());
}

void draw_button_image(const std::shared_ptr<portal::Image>& image, ImU32 tint_normal, ImU32 tint_hovered, ImU32 tint_pressed)
{
    draw_button_image(image, image, image, tint_normal, tint_hovered, tint_pressed, ImGui::GetItemRectMin(), ImGui::GetItemRectMax());
}
...
void render_window_outer_bounds(ImGuiWindow* window)
{
    ...
    auto get_resize_border_rect = [](const ImGuiWindow* window, const int border_n, const float perp_padding, const float thickness)
    {
    ...
    const char border_held = window->ResizeBorderHeld;
    if (border_held != -1)
    {
        const ImGuiResizeBorderDef& def = resize_border_def[border_held];
        const ImRect border_r = get_resize_border_rect(window, border_held, rounding, 0.0f);
        ImVec2 p1 = ImLerp(border_r.Min, border_r.Max, def.segment_n1);
        const float offsetX = def.inner_dir.x * rounding;
        const float offsetY = def.inner_dir.y * rounding;
        p1.x += 0.5f + offsetX;
        p1.y += 0.5f + offsetY;

        ImVec2 p2 = ImLerp(border_r.Min, border_r.Max, def.segment_n2);
        p2.x += 0.5f + offsetX;
        p2.y += 0.5f + offsetY;

        window->DrawList->PathArcTo(p1, rounding, def.outer_angle - IM_PI * 0.25f, def.outer_angle);
        window->DrawList->PathArcTo(p2, rounding, def.outer_angle, def.outer_angle + IM_PI * 0.25f);
...
bool UpdateWindowManualResize(ImGuiWindow* window, ImVec2& new_size, ImVec2& new_position)
{
    ...
    auto CalcWindowSizeAfterConstraint = [](ImGuiWindow* window, const ImVec2& size_desired)
    {
        ...
        if (!(window->Flags & (ImGuiWindowFlags_ChildWindow | ImGuiWindowFlags_AlwaysAutoResize)))
        {
            ImGuiWindow* window_for_height =
                (window->DockNodeAsHost && window->DockNodeAsHost->VisibleWindow) ? window->DockNodeAsHost->VisibleWindow : window;
            float decoration_up_height = window_for_height->TitleBarHeight + window_for_height->MenuBarHeight;
    ...
    auto CalcWindowAutoFitSize = [CalcWindowSizeAfterConstraint](ImGuiWindow* window, const ImVec2& size_contents)
    {
        ...
        else
        {
            ...
            ImVec2 size_auto_fit = ImClamp(
                size_desired,
                size_min,
                ImMax(size_min, {avail_size.x - style.DisplaySafeAreaPadding.x * 2.0f, avail_size.y - style.DisplaySafeAreaPadding.y * 2.0f}));
    ...
    auto CalcResizePosSizeFromAnyCorner =
            ...
            ImGuiWindow* window, const ImVec2& corner_target, const ImVec2& corner_norm, ImVec2* out_pos, ImVec2* out_size)
    {
        ...
        ImVec2 pos_max =
            ImLerp({window->Pos.x + window->Size.x, window->Pos.y + window->Size.y}, corner_target, corner_norm); // Expected window lower-right
    ...
    // - Not narrowing would mostly benefit the situation where OS windows _without_ decoration have a threshold for hovering when outside their
    // limits.
    //   This is however not the case with current backends under Win32, but a custom borderless window implementation would benefit from it.
    // - When decoration are enabled we typically benefit from that distance, but then our resize elements would be conflicting with OS resize
    // elements, so we also narrow.
    // - Note that we are unable to tell if the platform setup allows hovering with a distance threshold (on Win32, decorated window have such
    // threshold). We only clip interaction so we overwrite window->ClipRect, cannot call PushClipRect() yet as DrawList is not yet setup.
    ...
    for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++)
    {
        ...
        else if (held)
        {
            ...
            const float x = g.IO.MousePos.x - g.ActiveIdClickOffset.x +
                ImLerp(def.InnerDir.x * grip_hover_outer_size, def.InnerDir.x * -grip_hover_inner_size, def.CornerPosN.x);
            const float y = g.IO.MousePos.y - g.ActiveIdClickOffset.y +
                ImLerp(def.InnerDir.y * grip_hover_outer_size, def.InnerDir.y * -grip_hover_inner_size, def.CornerPosN.y);
        ...
        if (resize_grip_n == 0 || held || hovered)
            resize_grip_col[resize_grip_n] = ImGui::GetColorU32(
    ...
    for (int border_n = 0; border_n < resize_border_count; border_n++)
    {
        ...
        {
            ImVec2 clamp_min(
                border_n == ImGuiDir_Right ? visibility_rect.Min.x : -FLT_MAX, border_n == ImGuiDir_Down ? visibility_rect.Min.y : -FLT_MAX);
            ImVec2 clamp_max(
                border_n == ImGuiDir_Left ? visibility_rect.Max.x : +FLT_MAX, border_n == ImGuiDir_Up ? visibility_rect.Max.y : +FLT_MAX);
...
bool begin_menubar(const ImRect& bar_rectangle)
{
    ...
    ImRect bar_rect = ui::rect_offset(bar_rectangle, 0.0f, padding.y); // window->MenuBarRect();
    ImRect clip_rect(
...
void draw_border(ImRect rect, float thickness, float rounding, float offsetX, float offsetY)
{
    auto min = rect.Min;
    min.x -= thickness;
    min.y -= thickness;
