...
public:
	 ...
	 * @param caption The text to display
	 * @param selected_option The pointer to the variable set using the radio button.
	 * @param element_option Value set to the selectedOption when the button is selected.
	 * @returns True if adding item was successful
	 */
	bool radio_button(const char* caption, int32_t* selected_option, const int32_t element_option);
	...
	bool color_op(const std::string& caption, std::array<float, N>& color, float width = 0.0f, ImGuiColorEditFlags flags = 0)
	{
		...
		const bool res = color_op_impl<OP, N>(caption.c_str(), color.data(), flags);
		ImGui::PopItemWidth();
		if (res)
			dirty = true;
		return res;
