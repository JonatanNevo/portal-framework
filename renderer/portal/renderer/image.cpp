...
namespace utils
{
    static uint32_t get_vulkan_memory_type(const Renderer* renderer, const vk::MemoryPropertyFlags properties, const uint32_t type_bits)
    {
        const auto props = renderer->physical_device.getMemoryProperties();
...
Image::Image(Renderer* renderer, const std::string_view path) : renderer(renderer), file_path(path)
{
...
Image::Image(Renderer* renderer, const uint32_t width, const uint32_t height, const ImageFormat format, const void* data) : renderer(renderer),
    width(width),
    height(height),
    format(format)
{
...
{
    const vk::Device device = renderer->device;
    ...
    const vk::MemoryAllocateInfo alloc_info(
        req.size,
        utils::get_vulkan_memory_type(renderer, vk::MemoryPropertyFlagBits::eDeviceLocal, req.memoryTypeBits)
    );
...
void Image::set_data(const void* data)
{
    ...
    if (!staging_buffer)
    {
        ...
        const vk::MemoryAllocateInfo alloc_info(
            req.size,
            utils::get_vulkan_memory_type(renderer, vk::MemoryPropertyFlagBits::eHostVisible, req.memoryTypeBits)
        );
    ...
    renderer->flush_command_buffer(command_buffer);
}
...
void* Image::decode(const void* data, const size_t length, uint32_t& out_width, uint32_t& out_height)
{
    ...
    temp_data = stbi_load_from_memory(
