...
{
    Host::Host(const int port) : port(port), manager(ConnectionManager::get_instance()), sockets(manager->get_sockets()) {}

    Host::Host(ConnectionManager* manager, int port) : port(port), manager(manager), sockets(manager->get_sockets()) {}
    ...
    void Host::on_status_changed(SteamNetConnectionStatusChangedCallback_t* info)
    {
        ...
        case k_ESteamNetworkingConnectionState_Connecting:
        {
            ...
            {
                LOG_CORE_ERROR_TAG(
                    "Networking", "Host {} - Failed to set poll group for connection: {}", port, info->m_info.m_szConnectionDescription);
